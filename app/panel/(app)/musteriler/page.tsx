...
export default async function TeklifDetay({ params }: { params: { id: string } }) {
  ...
  const [teklif, sirket] = await Promise.all([
    prisma.teklif.findUnique({
      ...
      include: {
        ...
        yetkili: true, // Teklifte Seçilen Yetkili
        proje: true,
  ...
  // DÜZELTİLDİ: Tip-güvenli Müşteri Yetkilisi Belirleme
  const secilenYetkili = teklif.yetkili || (teklif.musteri.yetkililer && teklif.musteri.yetkililer.length > 0 ? teklif.musteri.yetkililer[0] : null);
      ...
      <div className="bg-yuzey border border-hat rounded-lg p-8 sm:p-12 print:border-0 print:p-0">
        ...
        <div className="grid sm:grid-cols-2 gap-6 mb-10 text-sm">
          <div className="space-y-1">
            ...
            {/* DÜZELTİLDİ: Tip-güvenli Müşteri Yetkilisi Gösterimi */}
            {secilenYetkili ? (
              <div className="bg-soguk-light/20 p-2.5 rounded border border-hat/50 text-xs mt-1.5 space-y-0.5">
                <p className="font-bold text-soguk-dim">
                  👤 Sayın: {secilenYetkili.ad} {secilenYetkili.unvan ? `(${secilenYetkili.unvan})` : ""}
                </p>
                {secilenYetkili.telefon && <p className="text-metin/70">📞 Tel: {secilenYetkili.telefon}</p>}
                {secilenYetkili.email && <p className="text-metin/70">✉️ Mail: {secilenYetkili.email}</p>}
              </div>
            ) : null}
          ...
          <div>
            ...
            {secilenYetkili && (
              <p className="text-xs text-metin/70 mt-0.5">
                Yetkili: {secilenYetkili.ad} {secilenYetkili.unvan ? `(${secilenYetkili.unvan})` : ""}
              </p>
            )}
            <div className="border-t border-hat mt-10 pt-2 text-metin/40 text-xs">İmza / Kaşe</div>
          </div>
