...
export default async function TekliflerSayfasi({
  ...
  searchParams: { musteri?: string; baslangic?: string; bitis?: string; min?: string; max?: string; proje?: string; no?: string };
}) {
  const [musteriler, tumTeklifler, sablonlar, markalar, urunler, projeler] = await Promise.all([
  ...
  return (
    <div>
          ...
          <>
            ...
            <div className="grid sm:grid-cols-2 gap-3 mb-5">
              ...
              <div>
                ...
                <select name="projeId" defaultValue="" className="focus-ring w-full border border-hat rounded-md px-3 py-2 text-sm">
            ...
            <div className="grid sm:grid-cols-4 gap-3 mb-5">
              ...
              <div>
                ...
                <select name="kdvDurumu" defaultValue="haric" className="focus-ring w-full border border-hat rounded-md px-3 py-2 text-sm">
            ...
            <label className="flex items-center gap-2 text-sm text-metin/80 mb-4">
              <input type="checkbox" name="birimFiyatGoster" value="hayir" className="accent-soguk" />
              PDF çıktısında birim fiyat ve iskonto sütunlarını gizle (sadece kalem toplamı görünsün)
            </label>
      ...
      <details className="bg-yuzey border border-hat rounded-lg mb-6" open={filtreVar}>
        ...
        <form method="get" className="p-5 pt-0 grid sm:grid-cols-6 gap-3 items-end">
          <div>
            <label className="block text-xs font-medium text-metin/60 mb-1">Teklif No</label>
            <input name="no" type="number" defaultValue={searchParams.no ?? ""} className="focus-ring w-full border border-hat rounded-md px-3 py-2 text-sm" placeholder="örn. 12" />
          </div>
          <div>
            <label className="block text-xs font-medium text-metin/60 mb-1">Müşteri</label>
            <select name="musteri" defaultValue={searchParams.musteri ?? ""} className="focus-ring w-full border border-hat rounded-md px-3 py-2 text-sm">
          ...
          <div>
            ...
            <input name="proje" defaultValue={searchParams.proje ?? ""} className="focus-ring w-full border border-hat rounded-md px-3 py-2 text-sm" placeholder="ara..." />
          </div>
          <div>
            <label className="block text-xs font-medium text-metin/60 mb-1">Başlangıç Tarihi</label>
            <input name="baslangic" type="date" defaultValue={searchParams.baslangic ?? ""} className="focus-ring w-full border border-hat rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-xs font-medium text-metin/60 mb-1">Bitiş Tarihi</label>
            <input name="bitis" type="date" defaultValue={searchParams.bitis ?? ""} className="focus-ring w-full border border-hat rounded-md px-3 py-2 text-sm" />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-xs font-medium text-metin/60 mb-1">Min Tutar</label>
              <input name="min" type="number" defaultValue={searchParams.min ?? ""} className="focus-ring w-full border border-hat rounded-md px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium text-metin/60 mb-1">Max Tutar</label>
              <input name="max" type="number" defaultValue={searchParams.max ?? ""} className="focus-ring w-full border border-hat rounded-md px-3 py-2 text-sm" />
            </div>
          </div>
          ...
          return (
            <div key={t.id} className="bg-yuzey border border-hat rounded-lg p-4 flex items-center justify-between gap-3">
              ...
              <div className="text-right flex items-center gap-3 shrink-0">
                ...
                <Link href={`/panel/teklifler/${t.id}/duzenle`} className="focus-ring text-xs text-metin/40 hover:text-soguk-dim">
                  Düzenle
                </Link>
